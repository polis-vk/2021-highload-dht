...
Разброс связан с записью данных из оперативной памяти на диск.

Возможная оптимизация - реализация асинхронной записи данных на диск 
для избежания блокировки пользовательских запросов.
...
Разброс можно объяснить отсутствием данных в оперативной памяти, что вынуждает искать их в физической.

Оптимизировать данную ситуацию можно увеличением кол-ва данных в оперативной памяти, либо добавлением кэша.
...
Из этого следует, что данный метод можно и нужно оптимизировать 
(увеличив размер SSTable, чтобы реже записывать данные на диск, 
либо сделав метод асинхронным, чтобы не блокировать пользовательские запросы).
...
Увеличить размер SSTable, чтоб реже записывать данные на диск или реализовать асинхронную запись на диск
для избежания блокировки пользовательских запросов.
...
Увеличить размер SSTable, чтобы реже обращаться за данными на диск или реализовать кэширование.