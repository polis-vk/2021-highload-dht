...
final class DataTransferChunk {
    ...
    private static final int CHUNK_NEXT_LINE_SIZE = 2 * CHUNK_NEXT_LINE.length + NEXT_LINE.length;
    ...
    private ByteBuffer buildChunk(final Record record) {
        ...
        final int totalSize = hexSize.length + key.length + value.length + CHUNK_NEXT_LINE_SIZE;
